        ...
        <div class="modal-content">
            ...
            <form method="post" id="form-reservas">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="pessoas" name="pessoas" placeholder="Nº de pessoas" value="001" required>
                            </div>
                        </div>
                    ...
                    <small>
                        <div style="text-align: center;" id="mensagem-reservas">
                        </div>
                    </small>
        ...
        <div class="modal-content">
            ...
            <form method="post" id="form-excluir">
                <div class="modal-body">
                    ...
                    <small>
                        <div style="text-align: center;" id="mensagem-excluir">
                        </div>
                    </small>
...
<div class="modal fade" id="modalReservaEmail" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form method="post" id="form-reservas-email">
                <div class="modal-body">
                    ...
                    <small>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td style="text-align: center; width:5%">
                                            <?php echo str_pad($res[$i]['pessoas'], 3, '0', STR_PAD_LEFT); ?>
                                        </td>
                                        ...
                                        <td style="text-align: center; width:5%">
                                            <a href="#" onclick="reservaEmail(
                                             '<?php echo $res[$i]['id'] ?>', 
                                            '<?php echo $res[$i]['email'] ?>', 
                                            '<?php echo $res[$i]['pessoas'] ?>',     '<?php echo $res[$i]['data_reserva'] ?>', 
                                            '<?php echo $res[$i]['mensagem'] ?>', event)"
                                                title="Confirmar Reserva" style="text-decoration: none">
                        ...
                        <div style="text-align: center;" id="mensagem-tab-reserva"></div>
                    </small>
...
<script type="text/javascript">
    $("#form-reservas").submit(function(event) {
        event.preventDefault();
...
<script type="text/javascript">
    ...
    function reservaEmail(id_res_email, email, pessoas, data, mensagem) {
        ...
        $.ajax({
            url: pag + "/confirmar_reserva_email.php",
            method: 'POST',
            data: {
                id_res_email,
                email,
            ...
            success: function(response) {
                $('#mensagem-tab-reserva').removeClass().text('');

                if (response.status === "sucesso") {
                    alert(response.mensagem);
                    $('#modalReservaEmail').modal('hide');
                    window.open(response.whatsapp, '_blank');
                    window.location = "index.php?pag=" + pag;
                } else {
                    alert(response.mensagem);
                    $('#modalReservaEmail').modal('hide');
                    $('#mensagem-tab-reserva').addClass('text-danger').text(response.mensagem);
                }
            },

            error: function(xhr, status, error) {
                $('#modalReservaEmail').modal('hide');
                alert('Nenhuma mesa disponível. Deseja outra data?');
                $('#mensagem-tab-reserva').addClass('text-danger').text('Nenhuma mesa disponível. Deseja outra data?');
            }
        });
    }
</script>

<!-- Fim Ajax Reservas por email -->
